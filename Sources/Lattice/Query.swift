...
public struct Query<T>: Sendable {
    ...
    private func anySubscript(appending key: CollectionSubscript) -> QueryNode {
        ...
        fatalError("Cannot add subscript to \(buildPredicate(node))")
    }
...
extension Query: ExpressibleByBooleanLiteral where T == Bool {
    public init(booleanLiteral value: BooleanLiteralType) {
        self = .init(QueryNode.constant(value))
    }
...
extension Optional: _QueryString where Wrapped: PrimitiveProperty, Wrapped: _QueryString { }

/// Tag protocol for all types that are compatible with `Binary`.
public protocol _QueryBinary { }
extension Data: _QueryBinary { }
extension Optional: _QueryBinary where Wrapped: PrimitiveProperty, Wrapped: _QueryBinary { }
    ...
    package func convertKeyPathsToEmbedded(rootPath: String? = nil) -> Query {
        func convertKeyPath(_ node: inout QueryNode) {
            ...
            case .comparison(operator: let op, var lhs, var rhs, let options):
                convertKeyPath(&lhs)
                convertKeyPath(&rhs)
                node = .comparison(operator: op, lhs, rhs, options: options)
            default: break
            }
        }
        var query = self
        convertKeyPath(&query.node)
        return query
    }
}
...
private struct SubqueryRewriter {
    ...
    static fileprivate func rewrite(_ node: QueryNode, _ counter: Int) -> (String, QueryNode) {
        ...
        guard let collectionName = rewriter.collectionName else {
            fatalError("Subqueries must contain a keypath starting with a collection.")
        }
