...
struct RelayConnectionSetupInput: @unchecked Sendable {
    ...
    let processFrame: @Sendable (WebSocket, ByteBuffer, Lattice) async -> Void
...
private final class RelayCatchUpReadState {
    ...
    func next(pageSpan: UInt64) -> RelayCatchUpStep {
        // This closes the native next() body, not destruction of submitted
        // captures/autorelease pools or release of the pool's per-file lane.
...
@RelayControlActor final class RelayConnectionSetup {
    ...
    private var isLive: Bool { !input.socket.isClosed && !input.state.revocation.isRevoked }
    ...
    private func subscribed(_ sub: PushSubscription?) {
        ...
        input.socket.eventLoop.execute {
            ...
            // Existing apply scheduling, frame ordering and byte accounting are
            // deliberately retained. Its async/generic admission is a separate
            // boundary; no claim about upload progress follows from this change.
