...
private final class ApplyTestGate: Sendable {
    ...
    var snapshot: State { state.withLockedValue { $0 } }
    func open() {
        state.withLockedValue { if $0.releasedNS == nil { $0.releasedNS = DispatchTime.now().uptimeNanoseconds } }
        release.signal()
    }
    func block() {
        state.withLockedValue { $0.enteredNS = DispatchTime.now().uptimeNanoseconds }
        // Never hold the diagnostic lock while blocking: timeout diagnostics
        // must remain readable while the worker is deliberately held.
        let timedOut = release.wait(timeout: .now() + 5) != .success
        state.withLockedValue {
            $0.timedOut = timedOut
            $0.finishedNS = DispatchTime.now().uptimeNanoseconds
        }
    }
}
