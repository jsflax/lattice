...
void maintenance(const std::string& path,const std::string& arm,bool traced,bool contender) {
    lattice_db owner(config(path)); owner.stop_audit_maintenance(); auto& db=owner.db();
    if(!contender) {
...
int main(int argc,char** argv) {
    try {
        ...
        else if(role=="maintenance" || role=="claim"){
            require(arm=="A1"||arm=="A2"||arm=="B","arm");maintenance(path,arm,role=="claim",role=="claim");
