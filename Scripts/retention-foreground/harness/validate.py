...
def summarize():
    ...
    for row in schedule:
        ...
        item['sampledWALMaxBytes']=max(samples,default=0)
        work_samples=[x['bytes']['-wal'] for x in data['fileSizeSamples']
                      if x['phase']=='work' and x['bytes']['-wal'] is not None]
        item['workPhaseSampledWALMaxBytes']=max(work_samples,default=0)
        if row['mode']!='claim':
    ...
    save(PACKET/'ANALYSIS.json',{'status':'sample analysis; final summary process cleanup not yet attested',
        ...
        'source':SHA,'tree':TREE,'runs':summaries,'pairedEffects':pairs,'numericSLO':None,
        'caveats':['Same source, recent/expired claim contrast; no code-speedup inference.',
                   '5ms sampled WAL maxima are lower bounds; all outliers retained.',
