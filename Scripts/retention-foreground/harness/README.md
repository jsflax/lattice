...
First hosted run35363934731 compiled the uniformO3 fixture successfully, then failed before any timed arm: the control used global/server apply while expecting per-synchronizer relay eligibility. Closed retained files show the two surviving remote entries marked globally synchronized and no per-sync rows. The corrected control uses apply_remote_changes_for with receiving ID ingress, verifies no echo after prune and exact relay IDs. All retention/value assertions, timing arms and guards are unchanged; original input/evidence remains preserved.
