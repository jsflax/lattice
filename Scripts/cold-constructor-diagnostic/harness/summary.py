...
COLD_CONSTRUCTOR = [6, 22, 23, 24, 25, 7]
...
def parse(text, arm):
    ...
    for row, label in zip(rows[1:], ['cold','warm']):
        ...
        if arm == 'on':
            ...
            if label == 'cold':
                components = {name: intervals[name] for name in
                              ['sqliteOpen','connectionSettings','vectorRegistration']}
...
def summarize(samples):
    ...
    return {'schema': 'cold-constructor-phases/1',
        'cppPhaseAttributionOnly': True, 'sampleProcesses': 6, 'runtime': runtime,
        'expectedRecords': {'cold':21, 'warm':15, 'capacity':24},
        'constructorComponents': ['sqliteOpen','connectionSettings','vectorRegistration','remainder'],
