...
class SummaryTests(unittest.TestCase):
    def test_valid(self):
        ...
        self.assertEqual(cold['intervalNs']['constructor'],50)
        self.assertEqual(cold['constructorComponentsNs'],
            {'sqliteOpen':10,'connectionSettings':10,'vectorRegistration':10,'remainder':20})
        self.assertEqual(cold['constructorShares'],
            {'sqliteOpen':0.2,'connectionSettings':0.2,'vectorRegistration':0.2,'remainder':0.4})
        self.assertEqual(value['expectedRecords'],{'cold':21,'warm':15,'capacity':24})
    ...
    def test_constructor_boundaries_reject_missing_duplicate_reordered_and_failed(self):
        ...
        def failed_vector(data): data[1]['records'][7]['fact']=1
    ...
    def test_zero_resolution_retained_without_inventing_shares(self):
        data=fixture()
        for record in data[1]['records'][3:9]:record['offsetNs']=30
        cold=parse(encode(data),'on')['samples'][0]
        self.assertEqual(cold['constructorComponentsNs'],
            {'sqliteOpen':0,'connectionSettings':0,'vectorRegistration':0,'remainder':0})
