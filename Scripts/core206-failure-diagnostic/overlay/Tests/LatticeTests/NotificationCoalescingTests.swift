...
struct NotificationCoalescingTests {
    ...
    func bulkUpdate_boundedDrainTasks_allObserversDeliver() async throws {
        ...
        let fired = FiredBox(captureCoalescing: true)
        var diagnosticTargets: [(ObjectIdentifier, Int64)] = []
        diagnosticTargets.reserveCapacity(200)
        ...
        do {
            ...
            for writer in writers {
                ...
                diagnosticTargets.append((ObjectIdentifier(twin), pk))
            }
        }

        ModelInstanceRegistry.shared._beginCoalescingDiagnostic(diagnosticTargets)
        ...
        ModelInstanceRegistry.shared._coalescingDiagnosticWaitBoundary("wait-begin", count: fired.count)
        ...
        ModelInstanceRegistry.shared._coalescingDiagnosticWaitBoundary("assertion-return", count: fired.count)
        ...
        for line in ModelInstanceRegistry.shared._freezeCoalescingDiagnostic(fired: fired.count, drains: drains) { print(line) }
...
private final class FiredBox: @unchecked Sendable {
    ...
    private let captureCoalescing: Bool
    init(captureCoalescing: Bool = false) { self.captureCoalescing = captureCoalescing }
    ...
    func insert(_ v: Int) {
        ...
        unique.insert(v)
        lock.unlock()
        if captureCoalescing { ModelInstanceRegistry.shared._coalescingDiagnosticCallback(Int64(v)) }
    }
