...
def run(args):
    ...
    assert args.mode in {'paired','held-reader'}, 'legacy in-SQL claim barrier is not admitted'
    assert args.source_mode in {'baseline','bounded'}
    ...
    def spawn(name, role, arm=None):
        ...
        if role == 'maintenance': argv.append(args.source_mode)
    ...
    def receive(name, value):
        ...
        if name == 'writer' and value['kind'] == 'ready':
            events.append({'kind':'writerStartSignaled','supervisorNS':time.monotonic_ns()})
            signal('writer')
    ...
    try:
        ...
        while True:
            ...
            if args.mode == 'claim':
                ...
                if phase == 'work' and ended('writer') and has('maintenance','tickDone'):
                    # No residual SQL is allowed until the writer is fully reaped.
                    events.append({'kind':'writerReapedBeforeDrain','supervisorNS':time.monotonic_ns()})
                    signal('maintenance'); phase = 'continuation'
                if phase == 'continuation' and ended('maintenance'):
                    ...
                    first=[x for x in events if x['kind']=='firstTickResidual']; assert len(first)==1
                    continuation=[x for x in events if x['kind']=='continuationTick']
                    expected=[7952,5904,3856,1808,0] if args.arm=='B' and args.source_mode=='bounded' else [0 if args.arm=='B' else 10000]
                    assert [first[0]['oldRows']]+[x['oldRows'] for x in continuation]==expected
                    assert [x['call'] for x in continuation]==list(range(2,len(expected)+1))
                    done=[x for x in events if x['kind']=='drainDone']; assert len(done)==1
                    assert done[0]['calls']==len(expected) and done[0]['automaticSchedulerMeasured'] is False
                    assert has('maintenance','maintenanceClosed')
        ...
        result = {'success':success,'error':error,'mode':args.mode,'arm':args.arm,
             'sourceMode':args.source_mode,'protocolVersion':2,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode',choices=['paired','held-reader'],required=True)
    parser.add_argument('--arm',choices=['A1','A2','B'],required=True)
    parser.add_argument('--source-mode',choices=['baseline','bounded'],required=True)
