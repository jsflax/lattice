// Versioned first-tick + manual-drain diagnostic. Product Core is unchanged.
// Historical single-tick 10,000-deletion assertion remains in the original harness.
...
void maintenance(const std::string& path,const std::string& arm,bool traced,bool contender, bool bounded) {
    lattice_db owner(config(path)); owner.stop_audit_maintenance(); auto& db=owner.db();
    if(!contender) {
    ...
    emit({{"kind","tickDone"},{"startNS",start},{"endNS",end},{"claims",trace.claims},{"prunes",trace.prunes},{"instrumented",traced}});
    // The first tick is measured exactly as before. Do not probe SQLite until
    // the supervisor confirms that all foreground updates and writer close ended.
    require(!traced && !contender, "legacy claim trace is not a drain experiment");
    command();
    auto residual = [&]() {
        const auto rows = db.query("SELECT count(*) AS n, COALESCE(MIN(id),0) AS first FROM AuditLog WHERE id<=10000");
        require(rows.size()==1, "residual shape");
        return std::pair<int64_t,int64_t>{std::get<int64_t>(rows[0].at("n")),std::get<int64_t>(rows[0].at("first"))};
    };
    auto remaining=residual();
    const int64_t first_expected=arm=="B" ? (bounded?7952:0) : 10000;
    require(remaining.first==first_expected && remaining.second==(first_expected==0?0:(arm=="B"?2049:1)), "first tick residual");
    emit({{"kind","firstTickResidual"},{"oldRows",remaining.first},{"firstOldID",remaining.second},{"observedNS",ns()}});
    int calls=1;
    const int64_t continuation_start=ns();
    while(arm=="B" && remaining.first!=0 && calls<5) {
        const auto call_start=ns(); owner.run_audit_retention_tick(); const auto call_end=ns(); ++calls;
        remaining=residual();
        const int64_t expected=std::max<int64_t>(0,10000-int64_t(calls)*2048);
        require(bounded && remaining.first==expected && remaining.second==(expected?10001-expected:0), "continuation residual");
        emit({{"kind","continuationTick"},{"call",calls},{"startNS",call_start},{"endNS",call_end},
              {"oldRows",remaining.first},{"firstOldID",remaining.second},{"observedNS",ns()}});
    }
    require(remaining.first==(arm=="B"?0:10000), "manual drain failed to converge");
    emit({{"kind","drainDone"},{"calls",calls},{"oldRows",remaining.first},{"continuationStartNS",continuation_start},
          {"firstEntryNS",start},{"endNS",ns()},{"automaticSchedulerMeasured",false}});
...
int main(int argc,char** argv) {
    try {
        ...
        if(role=="seed")seed(path);
            ...
            require(arm=="A1"||arm=="A2"||arm=="B","arm");
            require(argc==5 && (std::string(argv[4])=="baseline" || std::string(argv[4])=="bounded"),"source mode");
            maintenance(path,arm,false,false,std::string(argv[4])=="bounded");
