...
def main():
    ...
    binding.require(args.candidate_sdk_sha == config['candidateSDK'], 'candidate SDK is not reviewed source')
    admission = binding.admission_check(args.admission_json, args.admission_sha256, config)
    sys.modules['guarded_runner'] = guard
    build_proof = load_module('build_proof', PACKET / 'build_proof.py')
    ...
    for name in ('LATTICE_ACK_PATH_DIAGNOSTICS', 'LATTICE_OBSERVER_WORKER_DIAGNOSTICS',
                 'LATTICE_PERF_VARIANTS', 'LATTICE_PERF_SELECTED_BATCH', *binding.FORBIDDEN,
                 'CFLAGS', 'CXXFLAGS', 'CPPFLAGS', 'SWIFTFLAGS', 'OTHER_SWIFT_FLAGS',
                 'SWIFT_EXEC', 'SWIFT_EXEC_MANIFEST', 'SDKROOT', 'DYLD_INSERT_LIBRARIES'):
        env.pop(name, None)
    ...
    result = {'schemaVersion': 1, 'scope': 'exploratory same-VM-allocation A/A/B',
              ...
              'packetSealSHA256': args.seal_sha256, 'rootAdmissionSHA256': args.admission_sha256,
              'baselineProductPatchSHA256': config['baselineCorrectnessOverlay']['patchSHA256'],
              'variantScope': ['local', 'attached'], 'rawIterationCountExpected': 630,
    ...
    with guard.Interrupts() as interrupts:
        ...
        def command(label, argv, cwd, **kwargs):
            log = runner.run(label, argv, cwd=cwd, **kwargs)
            binding.command_check(receipts, label)
            return log
        ...
        def validate_sources(context, phase):
            ...
            allowed_paths = ('Package.resolved', config['baselineOverlayPath'], config['baselineCorrectnessOverlay']['path']) if context['label'] == 'baseline' else ('Package.resolved',)
            ...
            sdk_manifest = binding.manifest(PACKET, 'baseline-sdk-effective' if context['label'] == 'baseline' else 'candidate-sdk')
            core_manifest = binding.manifest(PACKET, context['label'] + '-core')
            source_check = binding.complete_sources(context['sdk'], sdk_manifest['files'],
                edited_core=context['core'] if context['label'] == 'candidate' else None,
                mutable_lock=context['label'] == 'candidate')
            core_check = binding.complete_sources(context['core'], core_manifest['files'])
            guard.save_json(receipts / (context['label'] + '-' + phase + '-complete-sources.json'),
                {'sdk': source_check, 'core': core_check, 'sdkManifestSHA256': guard.digest(PACKET / 'manifests' / ('baseline-sdk-effective.json' if context['label'] == 'baseline' else 'candidate-sdk.json')),
                 'coreManifestSHA256': guard.digest(PACKET / 'manifests' / (context['label'] + '-core.json'))})
        ...
        try:
            ...
            binding.require(original_source['files'] == binding.manifest(PACKET, 'baseline-sdk-pristine')['files'], 'pristine baseline SDK manifest mismatch')
            binding.require(original_core_source['files'] == binding.manifest(PACKET, 'baseline-core')['files'], 'pristine baseline Core manifest mismatch')
            ...
            for label in ('baseline', 'candidate'):
                ...
                if label == 'baseline':
                    ...
                    product = sdk / config['baselineCorrectnessOverlay']['path']
                    binding.require(guard.digest(product) == config['baselineCorrectnessOverlay']['preimageSHA256'], 'baseline correction preimage mismatch')
                    command('baseline-correction-check', ['git', 'apply', '--check', str(PACKET / 'baseline-product.patch')], sdk, timeout=60)
                    command('baseline-correction-apply', ['git', 'apply', str(PACKET / 'baseline-product.patch')], sdk, timeout=60)
                    binding.require(guard.digest(product) == config['baselineCorrectnessOverlay']['postimageSHA256'], 'baseline correction postimage mismatch')
                    shutil.copyfile(PACKET / 'PerfRefinementBenchmarks.swift', harness)
                    binding.complete_sources(sdk, binding.manifest(PACKET, 'baseline-sdk-effective')['files'])
                    result['baselineCorrectnessPatchApplied'] = True
                    ...
                    binding.require(source['tree'] == config['candidateSDKTree'] and source['files'] == binding.manifest(PACKET, 'candidate-sdk')['files'], 'candidate SDK source/tree mismatch')
            ...
            if candidate_core['tree'] != config['candidateCoreTree'] or candidate_core['files'] != binding.manifest(PACKET, 'candidate-core')['files']:
            ...
            for label, context in contexts.items():
                ...
                build = command(label + '-build-release-disabled-benchmark', command_args, context['sdk'], timeout=config['buildTimeoutSeconds'], require_full_timeout=True)
                proof = build_proof.make(build, context['sdk'], context['core'], context['home'] / 'scratch', config['baselineOverlayPath'],
                    temporary=root / 'tmp', map_receipts=receipts / (label + '-swift-output-maps'))
                proof['checkedCompilerFlags'] = binding.compiler_flags(proof, build, context['home'] / 'scratch', build_proof.native_arguments, label == 'candidate')
                ...
                context['proofHash'] = guard.digest(receipts / (label + '-compiler-inputs.json'))
            loaded = {}
            for run_label, implementation in [('A', 'baseline'), ('A2', 'baseline'), ('B', 'candidate')]:
                ...
                apply_env(context)
                validate_sources(context, 'before-' + run_label)
                build_proof.verify(context['proof'])
                binding.require(guard.digest(receipts / (implementation + '-compiler-inputs.json')) == context['proofHash'], 'compiler proof receipt changed')
                binding.packet_check(PACKET, args.seal_sha256)
                ...
                runner.env.update(LATTICE_PERF_REFINEMENT='1', LATTICE_PERF_RUN_DIR=str(run_directory),
                                  LATTICE_PERF_SOURCE_REVISION='git:' + context['sdkSHA'] + ';effective:' + guard.digest(PACKET / 'manifests' / ('baseline-sdk-effective.json' if implementation == 'baseline' else 'candidate-sdk.json')) + ';correctness-patch:' + (config['baselineCorrectnessOverlay']['patchSHA256'] if implementation == 'baseline' else 'none') + ';harness:' + config['frozenFiles']['PerfRefinementBenchmarks.swift'],
                ...
                loaded[run_label] = report.load_run(run_directory / 'result.json')
            ...
            # Use the frozen verifier/math unchanged, inside an explicitly nonqualifying envelope.
            ...
            guard.save_json(receipts / 'EXPLORATORY-COMPARISON.json', {'schemaVersion': 1,
                'qualifiesFrozenPhysicalHostGate': False, 'physicalHostIdentityVerified': False,
                'performanceTargetClaimed': False, 'scope': 'descriptive same-VM-allocation A/A/B; not physical-host qualification',
            ...
            guard.save_json(receipts / 'TARGET-OBSERVATIONS.json', binding.target_observations(comparison, loaded['B']))
            for context in contexts.values():
                apply_env(context)
                graph(context, 'after')
                validate_sources(context, 'final')
                build_proof.verify(context['proof'])
                binding.require(guard.digest(receipts / (context['label'] + '-compiler-inputs.json')) == context['proofHash'], 'final compiler proof receipt drift')
            ...
            with interrupts.hold():
                ...
                try:
                    binding.packet_check(PACKET, args.seal_sha256)
                    binding.admission_check(args.admission_json, args.admission_sha256, config)
                    command_audit = [binding.command_check(receipts, record['label']) for record in runner.records]
                    guard.save_json(receipts / 'FINAL-COMMAND-AUDIT.json', command_audit)
                    for context in contexts.values():
                        if 'proof' in context:
                            build_proof.verify(context['proof'])
