"""Baseline A instrumentation: exact original a05 fourteen; B keeps its seventeen.
...
def expected_tests(source):
    ...
    if len(names) != len(NATIVE_CASES) or set(names) != NATIVE_CASES:
        raise ValueError('reviewed probe source must contain exactly the fourteen named tests')
    return set(names)
...
def check_native_summary(summary):
    ...
    if (type(summary.get('count')) is not int or summary['count'] != len(NATIVE_CASES)
        ...
        raise ValueError('all fourteen exact native probe results are required before full calibration')
