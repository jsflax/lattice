...
def make(log, sdk, core, scratch, overlay):
    ...
    native = {}; swift_inputs = {}; link_graph = {}
    ...
    for line in log.read_text().splitlines():
        ...
        if tool in ('clang', 'clang++') and '-c' in argv:
            ...
            if source in expected_cpp:
                ...
                assert any(a in ('-O', '-O1', '-O2', '-O3', '-Os') for a in argv)
        ...
        if tool in ('clang', 'clang++', 'swiftc') and '-o' in argv and '-c' not in argv:
            ...
            for index, arg in enumerate(argv):
                if arg == '-filelist': lists.append(Path(argv[index + 1]).resolve())
                elif arg.startswith('@') and 'LinkFileList' in arg: lists.append(Path(arg[1:]).resolve())
            if not lists: continue
            members = set(); list_proof = []
            for path in lists:
                assert path.is_relative_to(scratch) and path.stat().st_size < 4 * 2**20
                listed = {str(Path(x).resolve()) for x in shlex.split(path.read_text())}
                assert all(Path(x).is_relative_to(scratch) and Path(x).is_file() for x in listed)
                members |= listed
                list_proof.append({'path': str(path), 'SHA256': guard.digest(path)})
            for arg in argv:
    ...
    proof.update(nativeObjects=native, swiftModules=swift_inputs, linkGraph=selected_links,
