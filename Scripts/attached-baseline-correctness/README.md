...
```
/opt/homebrew/opt/python@3.14/bin/python3.14 -B /Users/jason/localdev/lattice-perf-refinement-20260917/execution/preparation/attached-baseline-qualification/runner-004/qualify.py --root /Users/jason/localdev/lattice-perf-refinement-20260917/execution/validation/attached-baseline-d18-f8-001 --packet-seal-sha256 <reviewed PACKET-SEAL digest>
```
...
Revision003 adds bounded owned native argument response files and separates Darwin ld literal newline file lists from Swift response-file tokenization. Each response and file list remains hash-bound and verified. Non-command verbose-log lines are ignored before tokenization; required compile/link paths and source inventory remain unchanged. Two actual local Xcode6.4 format failures are preserved outside this packet. The changed parser passed read-only against the completed original physical build (15 Core/bridge objects, 2 response files, 85 link nodes). This confirms that retained format only: the substituted existing benchmark input is not the new attached overlay, and no native build/test was launched. Unknown target formats remain a fail-closed first-build gate.

Revision004 adds explicit `-Xswiftc -enable-testing` to both fresh Release `build --build-tests` commands after actual hosted Swift6.3.3 rejected an existing transport testable import. This preserves `-c release`, every optimization check, unchanged Swift overlay/product patch, and both-builds-before-tests ordering. Runner003 parser support is included unchanged; this run did not reach that parser. Prior runner002 remote result is preserved as compile failure, not a correctness red. The current 22 authored Python checks are not yet executed because local physical A/A2/B is active; prior runner003 had 21 executed checks. Root must run the changed checks before admission.
