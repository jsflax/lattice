...
class CalibrationTests(unittest.TestCase):
    ...
    def test_refresh_mode_conflicts_with_each_instrumented_mode_before_work(self):
        path = Path(__file__).with_name('run-development.py')
        spec = importlib.util.spec_from_file_location('calibration_refresh_cli_test', path)
        module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
        for mode in ('--sync-probe-qualification', '--sync-full-calibration'):
            argv = [str(path), '--root', str(self.root), '--core-sha', 'b' * 40,
                    '--test-timeout', '1800', '--recovery-refresh-qualification', mode]
            with self.subTest(mode=mode), patch('sys.argv', argv), contextlib.redirect_stderr(io.StringIO()), self.assertRaises(SystemExit) as error:
                module.main()
            self.assertEqual(error.exception.code, 2)
            self.assertFalse((self.root / 'receipts').exists())
