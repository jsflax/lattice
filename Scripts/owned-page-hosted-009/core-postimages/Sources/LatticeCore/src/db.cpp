...
database::~database() {
    ...
    if (db_) {
        if (mode_ == open_mode::read_write) {
            ...
            // Opted-in observation owners require an explicit trusted schema
            // acknowledgment for statistics DDL; teardown must not create it.
            if (!defer_destructor_optimize_) {
...
void database::drain_if_settled() {
    ...
    if (observation_delivery_scope::active_for(db_)) return;
...
database::database(database&& other) noexcept
    ...
    defer_destructor_optimize_ = other.defer_destructor_optimize_;
    observation_changes_valid_.store(other.observation_changes_valid_.load(std::memory_order_acquire));
    observation_changes_ = other.observation_changes_;
    observation_native_changes_ = other.observation_native_changes_;
    observation_native_total_ = other.observation_native_total_;
    other.observation_changes_valid_ = false;
    other.defer_destructor_optimize_ = false;
    other.db_ = nullptr;
}
...
int64_t database::changes() const {
    ...
    if (!observation_changes_valid_.load(std::memory_order_acquire)) return sqlite3_changes64(db_);
    auto* mutex = sqlite3_db_mutex(db_);
    sqlite3_mutex_enter(mutex);
    const auto native = sqlite3_changes64(db_);
    const auto result = observation_changes_valid_.load(std::memory_order_relaxed) && native == observation_native_changes_ &&
        sqlite3_total_changes64(db_) == observation_native_total_ ? observation_changes_ : native;
    sqlite3_mutex_leave(mutex);
    return result;
}
