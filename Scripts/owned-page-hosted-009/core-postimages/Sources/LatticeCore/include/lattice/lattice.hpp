...
protected:
    ...
    // PRIVATE owner selected-write predicates. begin() publishes this thread
    // only while its retained writer, topology lock and SQLite scope are held;
    // release_native() clears it before releasing either native lock. This
    // predicate lets that same thread avoid reacquiring its nonrecursive lock.
    bool owns_observation_topology_on_current_thread() const noexcept {
        return observation_topology_owner_.load(std::memory_order_acquire) == std::this_thread::get_id();
    }
    // Do not expose the model string or read it outside that owned scope.
    // Admission initializes it before begin() publishes ownership; a failed
    // enable can clear it only while no owned transaction is published.
    bool observation_owner_model_matches(const std::string& model) const noexcept {
        return owns_observation_topology_on_current_thread() && observation_owner_model_ == model;
    }
...
private:
    ...
    // Experimental internal owner remains off unless explicitly admitted.
    std::atomic<bool> observation_owner_enabled_{false};
    std::atomic<std::thread::id> observation_topology_owner_{};
    std::string observation_owner_model_; // immutable after successful admission
