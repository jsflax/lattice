...
class GuardedRunner:
    ...
    def __init__(self, root, receipts, env, interrupts, *, free_floor=FREE_FLOOR,
        ...
        self.control = detached_owner.Control()
        self.started = self.control.owner['startedMonotonic'] if self.control.owner else time.monotonic()
    ...
    def measure(self, log):
        return {'freeBytes': shutil.disk_usage(self.root).free,
                'packetBytes': allocated(self.root) + (allocated(self.control.directory) if self.control.directory else 0),
                'logBytes': (log.stat().st_size if log.exists() else 0) + ((self.control.directory / 'owner.log').stat().st_size if self.control.directory and (self.control.directory / 'owner.log').exists() else 0)}
    ...
    def cleanup(self, process, owned):
        proof = owned.close(process, self.overall_deadline, self.signal_grace)
        exists, error, method = self.group_state(process.pid)
        if error:
            proof['errors'].append(error)
        proof['groupGone'] = exists is False
        proof['groupProof'] = method
        return proof
    ...
    def run(self, label, argv, *, cwd, timeout=3600, require_full_timeout=False):
        ...
        try:
            ...
            while process.poll() is None:
                ...
                owned.refresh()
                if owned.ambiguous:
                    raise RuntimeError('runtime process ownership unproven')
                sample = self.measure(log)
            ...
            with self.interrupts.hold():
                if process is not None:
                    try:
                        if owned is None:
                            raise RuntimeError('command birth identity not acquired; no speculative signals')
                        record['cleanup'] = self.cleanup(process, owned)
                    except BaseException as error:
                        record['cleanup'] = {'groupGone': False, 'leaderReaped': False, 'ownedDescendantsGone': False,
                                             'proof': 'cleanup raised; proof missing', 'errors': [error_record(error)]}
                    record['exitCode'] = process.returncode
                else:
                    record['cleanup'] = {'groupGone': True, 'leaderReaped': True, 'ownedDescendantsGone': True, 'proof': 'no process started'}
                ...
                record['success'] = (record['started'] and record['exitCode'] == 0 and primary is None
                                     ...
                                     and record['cleanup']['leaderReaped'] and record['cleanup']['ownedDescendantsGone']
                                     and not record['cleanup'].get('errors'))
                try:
