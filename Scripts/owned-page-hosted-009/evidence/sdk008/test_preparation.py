...
class Preparation(unittest.TestCase):
    ...
    def test_source_custody_and_all_oracles_unchanged(self):
        ...
        config=json.loads((P/'CONFIG.json').read_text());old=Path(config['failedStandaloneSwift9Packet'])/'qualify.py'
        a=old.read_text();b=(P/'runtime_oracles.py').read_text()
        extract=lambda text:{n.name:ast.get_source_segment(text,n) for n in ast.parse(text).body if isinstance(n,ast.FunctionDef)}
        for name in ['analyze_runtime','physical_postimages']:self.assertEqual(extract(a)[name],extract(b)[name])
...
class RuntimeOutputDirectory(unittest.TestCase):
    ...
    def test_actual003_log_is_pinned_and_never_authorizes004_path(self):
        ...
        old=self.expected.replace('owned-page-sdk-qualification-008','owned-page-sdk-qualification-003')
