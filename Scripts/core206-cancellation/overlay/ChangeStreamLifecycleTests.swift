...
private final class CancellationPhaseDiagnostic: @unchecked Sendable {
    ...
    func finish(failed: Bool) {
        ...
        print("DIAGNOSTIC ChangeStreamCancellation: cutoff_ns=\(cutoff) points=\(String(decoding: encoded, as: UTF8.self)) body_exiting_is_not_task_completion=true diagnostic_threshold_exceeded=\(failed)")
    }
}
