...
OVERLAY_PATH = 'Tests/LatticeTests/ChangeStreamLifecycleTests.swift'
OVERLAY_BEFORE = '5ae8f5006ac64bf4fede33a97b4c28b0783cc5ef400c240f9195e87e7f86d2ea'
OVERLAY_AFTER = '921f0a2fe2610a617a08b3d5fd142989022ef78806420996d3fc6d595afc1f62'
...
class GuardedRunner:
    ...
    def run(self, label, argv, *, cwd, timeout=3600, require_full_timeout=False):
        ...
        try:
            ...
            output = log.open('xb')
            if label == 'full-test':
                telemetry = LinuxTelemetry(self.receipts / 'linux-scheduling.jsonl')
...
def load_binding(path):
    ...
    if (binding.get('schema') != 1
            ...
            or (binding.get('coreVersion'), binding.get('coreCommit'), binding.get('coreTree')) not in (
                ('2.0.5', '559c496d7997677d06cc6d1f7a7a87abc440c210', '0ab83c94e7a95fb53ba907025f76263b3e78e353'),
                ('2.0.6', 'db6bf8b97d7af22db4c1224f17bbdea80a43cc57', '781cf09659a06475d88c1f33122c2130142e93c1'))):
        raise ValueError('diagnostic pair is limited to SDK0461 with exact Core559 or db6')
...
def verify_overlay(sdk):
    actual = digest(sdk / OVERLAY_PATH)
    if actual != OVERLAY_AFTER:
        raise ValueError('diagnostic overlay postimage changed')
    return {'path': OVERLAY_PATH, 'sha256': actual}


def apply_overlay(sdk, receipts):
    source = Path(__file__).parent / 'overlay/ChangeStreamLifecycleTests.swift'
    if digest(sdk / OVERLAY_PATH) != OVERLAY_BEFORE or digest(source) != OVERLAY_AFTER:
        raise ValueError('diagnostic overlay preimage/postimage mismatch')
    (sdk / OVERLAY_PATH).write_bytes(source.read_bytes())
    save_json(receipts / 'sdk-diagnostic-overlay.json', {
        'path': OVERLAY_PATH, 'beforeSHA256': OVERLAY_BEFORE, 'afterSHA256': OVERLAY_AFTER,
        'scope': 'test diagnostic emits existing phase record on pass too; original test oracle/membership unchanged',
        'publishedSDKModifiedForDiagnostic': True, 'releaseSDKSourceChanged': False})
    verify_overlay(sdk)


def configure_pair_limits(runner, root, aggregate_root, work_deadline, overall_deadline):
    ...
    if (aggregate == allowed or not aggregate.is_relative_to(allowed)
        ...
        raise ValueError('arm must be a strict child of an owned aggregate localdev root')
    if (not all(math.isfinite(x) for x in (work_deadline, overall_deadline))
            or work_deadline <= now or work_deadline > overall_deadline - FINALIZATION_RESERVE
            or overall_deadline > now + 340 * 60):
        raise ValueError('invalid bounded pair deadlines')
    runner.aggregate_root = aggregate
    runner.work_deadline = min(runner.work_deadline, work_deadline)
    runner.overall_deadline = min(runner.overall_deadline, overall_deadline)
...
def main():
    ...
    parser.add_argument('--pair-work-deadline', type=float, required=True)
    parser.add_argument('--pair-overall-deadline', type=float, required=True)
    args = parser.parse_args()
    if platform.system() != 'Linux' or args.test_timeout != 5400:
        raise ValueError('this matched diagnostic packet is Linux-only with unchanged 5400s allowance')
    ...
    result = {'scope': 'matched cancellation diagnostic with explicit test-only overlay; not standalone release acceptance',
              ...
              'diagnosticOverlaySHA256': OVERLAY_AFTER,
              'aggregateRoot': str(args.aggregate_root),
              'pairWorkDeadline': args.pair_work_deadline, 'pairOverallDeadline': args.pair_overall_deadline,
    ...
    with Interrupts() as interrupts:
        ...
        configure_pair_limits(runner, root, args.aggregate_root, args.pair_work_deadline, args.pair_overall_deadline)
        ...
        try:
            ...
            final_sdk = authenticate_repository(runner, 'sdk-final', sdk, sdk_sha, allowed_changes=('Package.resolved', OVERLAY_PATH))
            final_core = authenticate_repository(runner, 'core-final', core, core_sha)
            initial_sdk_files = {k: v for k, v in sdk_inputs['files'].items() if k not in ('Package.resolved', OVERLAY_PATH)}
