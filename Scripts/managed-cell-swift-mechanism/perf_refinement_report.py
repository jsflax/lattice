...
def load_run(path, *, expected_variants=("local", "attached")):
    # Caller must opt in to local-only; an absent attached run never passes the
    # default complete-contract reporter merely because its manifest omits it.
    require(tuple(expected_variants) in (("local", "attached"), ("local",)), "unsupported variant selection")
    ...
    require(manifest["variants"] == list(expected_variants), "fixture variants differ from explicit selection")
    ...
    require(len(samples) == len(expected_variants) * (measured + warmups), "sample count mismatch")
    variants = {}
    checksum_contract = {}
    ...
    if "attached" in expected_variants:
    ...
    expected_fixtures = {"local/master/main.sqlite"}
    if "attached" in expected_variants:
        expected_fixtures.update(("attached/master/main.sqlite", "attached/master/attached.sqlite"))
    require(set(fixtures) == expected_fixtures, "preserved master database inventory differs")
    for variant in manifest["variants"]:
...
def main():
    ...
    parser.add_argument("--variant-scope", choices=("all", "local-only"), default="all")
    ...
    expected_variants = ("local",) if args.variant_scope == "local-only" else ("local", "attached")
    ...
    report = {
        ...
        "variantScope": args.variant_scope,
        "fullContractQualified": False,
        "attachedQualificationClaimed": False,
    ...
    if singles:
        run = load_run(args.run, expected_variants=expected_variants)
        report["runs"] = {"run": run}
        ...
        a, repeat, candidate = (load_run(x, expected_variants=expected_variants) for x in (args.baseline, args.repeat, args.candidate))
