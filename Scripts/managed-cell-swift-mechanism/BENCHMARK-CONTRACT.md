...
## Explicit local-only evidence lane

This separately selected lane uses `LATTICE_PERF_VARIANTS=local`; absence keeps the original `local,attached` inventory. Other values reject. The manifest records exactly `["local"]`, and the reporter requires `--variant-scope local-only` (or the runner's explicit equivalent). Default reporting still requires both variants. All A/A/B inputs use the same selected inventory; mixed scope rejects.

The local 10,000-row fixture, six live fields, Read100/Update11 bodies, transactions, 100 measured samples, five warmups, before/after checksums, page/SQL invariants, percentile math and A/A noise comparison remain unchanged. The local 2× target remains unchanged. These measurements cannot satisfy the attached or full-plan target. `fullContractQualified`, `attachedQualificationClaimed`, and the hosted-VM physical-host/performance claim flags remain false.

Original attempt 35352719982 remains failed: A completed its 105 local iterations, then the first attached warmup failed displayed-value/order validation. A2 and B never started. Its immutable ANALYSIS/SEAL and raw artifact are retained; no correction, waiver, inferred sample or rewritten baseline is permitted. The earlier fixture-validation failure 35346193210 also remains preserved. This lane creates new independent A/A/B results and does not turn that incomplete original A into a complete result. The attached target/comparison decision remains open.
