...
COLD_CONSTRUCTOR = [6, 22, 23, 26, 27, 28, 29, 30, 31, 24, 25, 7]
PAIRS = {'acquisition': (1,16), 'eligibilityEviction': (1,2), 'poolSelection': (2,3),
         'constructor': (6,7), 'constructorPrefix': (6,22), 'sqliteOpen': (22,23),
         'connectionSettings': (23,24), 'vectorRegistration': (24,25),
         'settingsPrefix': (23,26), 'busyHandling': (26,27), 'foreignKeys': (27,28),
         'cacheSetting': (28,29), 'mmapSetting': (29,30), 'tempSetting': (30,31),
         'scanstatusTail': (31,24),
         'constructorTail': (25,7), 'begin': (10,11), 'pin': (12,13),
...
def parse(text, arm):
    ...
    for row, label in zip(rows[1:], ['cold','warm']):
        ...
        assert type(row['sql']) is int and row['sql'] == (7 if label == 'cold' else 3)
        ...
        if arm == 'on':
            ...
            assert len(records) == len(tags) <= 27 and [r['tag'] for r in records] == tags
            prior = -1; by_tag = {}
            for record in records:
                assert set(record) == {'tag','offsetNs','fact'}
                assert type(record['tag']) is int
            ...
            assert all(r['fact'] == 0 for r in records if r['tag'] not in [3,16,21,24])
            if label == 'cold': assert by_tag[24]['fact'] in [0,1]
            ...
            item.update(intervalNs=intervals, outerResidualNs=residual,
                        acquisitionUnattributedNs=intervals['acquisition']-sum(intervals.get(k,0) for k in
                            ['eligibilityEviction','poolSelection','constructor','begin','pin','publication']))
            assert item['acquisitionUnattributedNs'] >= 0
            if label == 'cold':
                settings = {name: intervals[name] for name in
                            ['busyHandling','foreignKeys','cacheSetting','mmapSetting','tempSetting','scanstatusTail']}
                assert sum(settings.values()) + intervals['settingsPrefix'] == intervals['connectionSettings']
                components = {'sqliteOpen':intervals['sqliteOpen'], **settings,
                              'vectorRegistration':intervals['vectorRegistration']}
                components['remainder'] = (intervals['constructorPrefix'] + intervals['constructorTail'] +
                                           intervals['settingsPrefix'])
                assert all(value >= 0 for value in components.values())
                assert sum(components.values()) == intervals['constructor']
                item['constructorComponentsNs'] = components
                item['scanstatusCallCompiled'] = bool(by_tag[24]['fact'])
                # Zero-resolution intervals are retained, never normalized or
                # discarded. Shares are undefined if the outer duration is zero.
                item['constructorShares'] = ({name: value / intervals['constructor']
                    for name,value in components.items()} if intervals['constructor'] else None)
...
def summarize(samples):
    ...
    return {'schema': 'cold-settings-phases/1',
        'cppPhaseAttributionOnly': True, 'sampleProcesses': 6, 'runtime': runtime,
        'expectedRecords': {'cold':27, 'warm':15, 'capacity':27},
        'constructorComponents': ['sqliteOpen','busyHandling','foreignKeys','cacheSetting','mmapSetting',
                                  'tempSetting','scanstatusTail','vectorRegistration','remainder'],
