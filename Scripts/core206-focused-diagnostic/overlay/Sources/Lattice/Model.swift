...
final class ModelInstanceRegistry: @unchecked Sendable {
    ...
    func notifyChange(databasePath: String, tableName: String, primaryKey: Int64, propertyName: String, excludingInstanceId: ObjectIdentifier? = nil) {
        ...
        let refs = instances[key] ?? []
        lock.unlock()
        _captureNotify(key, propertyName: propertyName, refs: refs.count)
    ...
    // Qualification-only scalar capture. No model, actor, backend, task or
    // callback is stored. All mutable capture state uses notifyLock; registry
    // keys are copied under lock and that lock is released before notifyLock.
    private struct CoalescingEvent {
        let stage: String
        let at: UInt64
        let row: Int64
        let batch: UInt64
    ...
    private struct CoalescingCapture {
        var active = false
        var targets: [ObjectIdentifier: Int64] = [:]
        var keys: [InstanceKey: Int64] = [:]
        var events: [CoalescingEvent] = []
        var dropped = 0
        var batches: UInt64 = 0
        var runs: UInt64 = 0
        var currentBatch: UInt64 = 0
        var currentRun: UInt64 = 0
        var currentActor = "none"
        var currentPhase = "armed"
        var currentPhaseAt: UInt64 = 0
        var currentRunTotal = 0
        var currentRunTargets: Int? = nil
        var initialPending = 0
        var notifyCalls = 0
        var enqueued = 0
        var deduplicated = 0
        var taken = 0
        var triggerReturned = 0
        var missingWeak = 0
        var callbackCount = 0
        var callbackRows: Set<Int64> = []
        var pending: Set<ObjectIdentifier> = []
        var inFlight: Set<ObjectIdentifier> = []
        var beganAt: UInt64 = 0
        var frozenAt: UInt64 = 0
    }
    private let coalescingCaptureEnabled =
        ProcessInfo.processInfo.environment["LATTICE_OBSERVER_WORKER_DIAGNOSTICS"] == "1"
    private var coalescingCapture = CoalescingCapture()

    func _beginCoalescingDiagnostic(_ targets: [(ObjectIdentifier, Int64)]) {
        guard coalescingCaptureEnabled, targets.count == 200 else { return }
        var capture = CoalescingCapture()
        for (identity, row) in targets { capture.targets[identity] = row }
        guard capture.targets.count == 200, Set(capture.targets.values).count == 200 else { return }
        lock.lock()
        for (identity, row) in targets {
            if let key = registeredKeys[identity] { capture.keys[key] = row }
        }
        lock.unlock()
        capture.events.reserveCapacity(2048)
        capture.active = true
        capture.beganAt = DispatchTime.now().uptimeNanoseconds
        notifyLock.lock()
        // Preserve the scalar position of a drainer already active when armed.
        capture.batches = coalescingCapture.batches
        capture.runs = coalescingCapture.runs
        capture.currentBatch = coalescingCapture.currentBatch
        capture.currentRun = coalescingCapture.currentRun
        capture.currentActor = coalescingCapture.currentActor
        capture.currentPhase = coalescingCapture.currentPhase
        capture.currentPhaseAt = coalescingCapture.currentPhaseAt
        capture.currentRunTotal = coalescingCapture.currentRunTotal
        // Exactly 200 set lookups; do not scan or retain the global queue.
        for identity in capture.targets.keys {
            if pendingSeen.contains(_PendingKey(id: identity, propertyName: "age")) {
                capture.pending.insert(identity)
            }
        }
        capture.initialPending = capture.pending.count
        coalescingCapture = capture
        _captureEvent("armed", count: capture.targets.count)
        notifyLock.unlock()
    }

    // Called only with notifyLock held. Fixed stage labels; no output/callout.
    private func _captureEvent(_ stage: String, row: Int64 = 0, count: Int = 0) {
        guard coalescingCapture.active else { return }
        guard coalescingCapture.events.count < 2048 else {
            coalescingCapture.dropped += 1
            return
        }
        coalescingCapture.events.append(CoalescingEvent(stage: stage,
            at: DispatchTime.now().uptimeNanoseconds, row: row,
            batch: coalescingCapture.currentBatch, run: coalescingCapture.currentRun,
            actor: coalescingCapture.currentActor, count: count))
    }

    private func _captureNotify(_ key: InstanceKey, propertyName: String, refs: Int) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        if coalescingCapture.active, propertyName == "age", let row = coalescingCapture.keys[key] {
            coalescingCapture.notifyCalls += 1
            _captureEvent("notify", row: row, count: refs)
        }
    }

    // Called only from enqueue/take while notifyLock is already held.
    private func _captureEnqueue(_ value: _PendingNotification, inserted: Bool) {
        guard coalescingCapture.active, value.propertyName == "age",
              let row = coalescingCapture.targets[value.ref.objectIdentifier] else { return }
        if inserted {
            coalescingCapture.enqueued += 1
            coalescingCapture.pending.insert(value.ref.objectIdentifier)
        } else { coalescingCapture.deduplicated += 1 }
        _captureEvent(inserted ? "enqueue" : "dedup", row: row)
    }

    private func _captureTake(_ values: [_PendingNotification]) {
        guard coalescingCaptureEnabled else { return }
        coalescingCapture.batches += 1
        coalescingCapture.currentBatch = coalescingCapture.batches
        guard coalescingCapture.active else { return }
        for value in values where value.propertyName == "age" {
            let identity = value.ref.objectIdentifier
            if let row = coalescingCapture.targets[identity] {
                coalescingCapture.pending.remove(identity)
                coalescingCapture.inFlight.insert(identity)
                coalescingCapture.taken += 1
                _captureEvent("take", row: row)
            }
        }
    }

    private func _captureRun(_ values: [_PendingNotification], actor: String) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        coalescingCapture.runs += 1
        coalescingCapture.currentRun = coalescingCapture.runs
        coalescingCapture.currentActor = actor
        coalescingCapture.currentRunTotal = values.count
        let targetCount: Int? = !coalescingCapture.active ? nil : values.reduce(0) {
            $0 + ($1.propertyName == "age" && coalescingCapture.targets[$1.ref.objectIdentifier] != nil ? 1 : 0)
        }
        coalescingCapture.currentRunTargets = targetCount
        coalescingCapture.currentPhase = "actor-request"
        coalescingCapture.currentPhaseAt = DispatchTime.now().uptimeNanoseconds
        _captureEvent("actor-request", count: coalescingCapture.currentRunTargets ?? -1)
    }

    private func _captureRunPhase(_ phase: String) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        coalescingCapture.currentPhase = phase
        coalescingCapture.currentPhaseAt = DispatchTime.now().uptimeNanoseconds
        _captureEvent(phase, count: coalescingCapture.currentRunTargets ?? -1)
    }

    private func _captureTarget(_ value: _PendingNotification, phase: String, sent: Bool? = nil) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        coalescingCapture.currentPhase = phase
        coalescingCapture.currentPhaseAt = DispatchTime.now().uptimeNanoseconds
        ...
        if phase == "trigger-return" {
            coalescingCapture.triggerReturned += 1
            coalescingCapture.inFlight.remove(value.ref.objectIdentifier)
        }
        if sent == false { coalescingCapture.missingWeak += 1 }
        _captureEvent(phase, row: row, count: sent == nil ? -1 : (sent == true ? 1 : 0))
    }

    func _coalescingDiagnosticCallback(_ row: Int64) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        guard coalescingCapture.active, coalescingCapture.targets.values.contains(row) else { return }
        coalescingCapture.callbackCount += 1
        coalescingCapture.callbackRows.insert(row)
        _captureEvent("callback", row: row)
    }

    func _coalescingDiagnosticWaitBoundary(_ phase: String, count: Int) {
        guard coalescingCaptureEnabled else { return }
        notifyLock.lock(); defer { notifyLock.unlock() }
        _captureEvent(phase, count: count)
    }

    func _freezeCoalescingDiagnostic(fired: Int, drains: Int) -> [String] {
        guard coalescingCaptureEnabled else { return [] }
        notifyLock.lock()
        guard coalescingCapture.active else { notifyLock.unlock(); return [] }
        coalescingCapture.frozenAt = DispatchTime.now().uptimeNanoseconds
        coalescingCapture.active = false
        let frozen = coalescingCapture
        let scheduled = drainScheduled
        let queueCount = pendingNotifications.count
        notifyLock.unlock()
        let runTargets = frozen.currentRunTargets.map { String($0) } ?? "unknown"
        var lines = ["DIAGNOSTIC ModelCoalescing: targets=\(frozen.targets.count) registered_keys=\(frozen.keys.count)"
            + " began_ns=\(frozen.beganAt) frozen_ns=\(frozen.frozenAt) events=\(frozen.events.count) dropped=\(frozen.dropped) cap=2048"
            + " notify=\(frozen.notifyCalls) enqueued=\(frozen.enqueued) dedup=\(frozen.deduplicated) taken=\(frozen.taken)"
            + " trigger_returned=\(frozen.triggerReturned) missing_weak_calls=\(frozen.missingWeak)"
            + " callbacks=\(frozen.callbackCount) callback_rows=\(frozen.callbackRows.count)"
            + " initial_pending_targets=\(frozen.initialPending) prearm_inflight_membership=unknown"
            + " pending_targets=\(frozen.pending.count) inflight_targets=\(frozen.inFlight.count)"
            + " total_pending=\(queueCount) drain_scheduled=\(scheduled) batches=\(frozen.batches) runs=\(frozen.runs)"
            + " current_batch=\(frozen.currentBatch) current_run=\(frozen.currentRun) actor=\(frozen.currentActor)"
            + " phase=\(frozen.currentPhase) phase_ns=\(frozen.currentPhaseAt) run_total=\(frozen.currentRunTotal) run_targets=\(runTargets)"
            + " observed_fired=\(fired) observed_drains=\(drains)"
            + " clock=dispatch_uptime frozen=true late_events_unobserved=true"]
        let pendingRows = frozen.pending.compactMap { frozen.targets[$0] }.sorted()
        let inFlightRows = frozen.inFlight.compactMap { frozen.targets[$0] }.sorted()
        lines.append("DIAGNOSTIC ModelCoalescingRows: targets=\(frozen.targets.values.sorted()) callbacks=\(frozen.callbackRows.sorted()) pending=\(pendingRows) inflight=\(inFlightRows)")
        for (index, event) in frozen.events.enumerated() {
            lines.append("DIAGNOSTIC ModelCoalescingEvent: index=\(index) stage=\(event.stage) uptime_ns=\(event.at)"
                + " row=\(event.row) batch=\(event.batch) run=\(event.run) actor=\(event.actor) count=\(event.count)")
        }
        return lines
    }
    ...
    private func _enqueueNotification(_ pending: _PendingNotification) {
        var schedule = false
        notifyLock.lock()
        let inserted = pendingSeen.insert(_PendingKey(id: pending.ref.objectIdentifier, propertyName: pending.propertyName)).inserted
        if inserted {
            pendingNotifications.append(pending)
        }
        _captureEnqueue(pending, inserted: inserted)
        if !drainScheduled {
            drainScheduled = true
    ...
    private func _takePendingBatch() -> [_PendingNotification]? {
        ...
        if pendingNotifications.isEmpty {
            ...
            if coalescingCaptureEnabled {
                coalescingCapture.currentPhase = "idle"
                coalescingCapture.currentPhaseAt = DispatchTime.now().uptimeNanoseconds
            }
    ...
    private func _drainPendingNotifications() async {
        while true {
            ...
            while index < batch.count {
                ...
                _captureRun(run, actor: isolation.map { ObjectIdentifier($0) == ObjectIdentifier(MainActor.shared) ? "main" : "other" } ?? "nil")
                if let isolation {
                    await isolation.invoke { _ in
                        self._captureRunPhase("actor-enter")
                        for pending in run {
                            self._captureTarget(pending, phase: "willChange-before")
                            let sentWillChange: ()? = pending.ref.instance?._objectWillChange_send()
                            self._captureTarget(pending, phase: "willChange-return", sent: sentWillChange != nil)
                            self._captureTarget(pending, phase: "trigger-before")
                            let sentTrigger: ()? = pending.ref.instance?._triggerObservers_send(keyPath: pending.propertyName)
                            self._captureTarget(pending, phase: "trigger-return", sent: sentTrigger != nil)
                        }
                        self._captureRunPhase("actor-exit")
                    }
                } else {
                    // No hop target: deliver on the drain task. Both
                    // notification systems are thread-safe entry points.
                    _captureRunPhase("actor-enter")
                    for pending in run {
                        _captureTarget(pending, phase: "willChange-before")
                        let sentWillChange: ()? = pending.ref.instance?._objectWillChange_send()
                        _captureTarget(pending, phase: "willChange-return", sent: sentWillChange != nil)
                        _captureTarget(pending, phase: "trigger-before")
                        let sentTrigger: ()? = pending.ref.instance?._triggerObservers_send(keyPath: pending.propertyName)
                        _captureTarget(pending, phase: "trigger-return", sent: sentTrigger != nil)
                    }
                    _captureRunPhase("actor-exit")
                }
                _captureRunPhase("drain-resumed")
                index = end
            }
        }
    }
