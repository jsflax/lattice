...
FOCUSED_FILTER = '|'.join(FOCUSED_NAMES)
...
def verify_focused_membership(log):
    """Exit zero alone is not selected-test acceptance; require the actual four cases."""
    starts, passes, summaries = [], [], []
    with log.open(encoding='utf-8', errors='strict') as source:
        for line in source:
            line = line.rstrip('\r\n')
            start = re.fullmatch(r'◇ Test (.+) started\.', line)
            passed = re.fullmatch(r'✔ Test (.+) passed after [0-9.]+ seconds\.', line)
            summary = re.fullmatch(r'✔ Test run with (\d+) tests? in (\d+) suites? passed after [0-9.]+ seconds\.', line)
            if start:
                starts.append(start.group(1))
            if passed and summary is None:
                passes.append(passed.group(1))
            if summary:
                summaries.append((int(summary.group(1)), int(summary.group(2))))
            if ('No matching test cases were run' in line or line.startswith(('✘ ', '↷ '))
                    or (line.startswith('➜ Test ') and 'skipped' in line)
                    or (line.startswith('Test Suite ') and ' failed ' in line)):
                raise ValueError('focused membership: skip, issue, failure or zero-selection warning')
            if len(starts) > 4 or len(passes) > 4 or len(summaries) > 1:
                raise ValueError('focused membership: extra test execution or summary')
    expected = sorted(name + '()' for name in FOCUSED_NAMES)
    if sorted(starts) != expected or sorted(passes) != expected or summaries != [(4, 2)]:
        raise ValueError('focused membership: exact four starts/passes and four-test/two-suite terminal summary required')
    return {'expectedTests': expected, 'startedTests': starts, 'passedTests': passes,
            'terminalTests': 4, 'terminalSuites': 2, 'skips': 0,
            'logSHA256': digest(log), 'accepted': True}


def run_test_invocations(runner, common, sdk, root, outcomes):
    """One focused invocation only; retain its first command or evidence failure."""
    first_error = None
    for label, suffix in [('focused-test', ['--filter', FOCUSED_FILTER])]:
        entry = {'label': label, 'expectedParentTests': 4,
                 'expectedKnownSkips': 0,
        ...
        try:
            ...
            membership = verify_focused_membership(runner.receipts / (label + '.log'))
            if membership['logSHA256'] != record.get('logSHA256'):
                raise ValueError('focused membership: log differs from actual command receipt')
            membership_path = runner.receipts / 'focused-membership.json'
            save_json(membership_path, membership)
            entry['membershipSHA256'] = digest(membership_path)
        except BaseException as error:
        ...
        if not safe or runner.interrupts.received:
            entry['sequenceStopped'] = 'incomplete settlement or interruption'
            return first_error or RuntimeError(entry['sequenceStopped']), False
        # There is no second test invocation. Preserve capture/membership failure
        # while allowing the existing guarded after-graph/source checks to run.
...
def main():
    ...
    result = {'scope': 'one focused-only candidate diagnostic; prior full stage is assessed separately',
