...
def main():
    ...
    with guard.Interrupts() as interrupts:
        ...
        def custody():
            if proof is None:return {'profileBuildAvailable':False}
            build_proof.verify(proof);profile_proof.verify(supplement)
            if context.get('retainedInputs') is not None:
                binding.verify_retained_inputs(context['retainedInputs'])
                binding.require(guard.digest(receipts/'RETAINED-COMPILER-INPUTS.json')==context['retainedInputsHash'],'compiler input receipt drift')
            binding.require(guard.digest(receipts/'COMPILER-PROOF.json')==context['proofHash'],'compiler proof drift')
            binding.require(guard.digest(receipts/'PROFILE-PROOF.json')==context['profileProofHash'],'profile proof drift')
            return {'binarySHA256':guard.digest(Path(proof['binary'])),'compilerProofSHA256':context['proofHash'],
                    'profileProofSHA256':context['profileProofHash']}
        def host(label):
        ...
        try:
            ...
            guard.save_json(receipts/'COMPILER-PROOF.json',proof);guard.save_json(receipts/'PROFILE-PROOF.json',supplement)
            context['proofHash']=guard.digest(receipts/'COMPILER-PROOF.json');context['profileProofHash']=guard.digest(receipts/'PROFILE-PROOF.json')
