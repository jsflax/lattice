...
def run(args):
    ...
    try:
        if args.mode == 'claim':
            ...
            spawn('maintenance', 'maintenance-phases', args.arm)
        ...
        result = {'success':success,'error':error,'mode':args.mode,'arm':args.arm,
             ...
             'claim':'one instrumented maintenance trace; timestamps perturbed by tracing;5ms WAL maxima are sampled lower bounds'}
        try:
            save(receipt, result)
        except BaseException as caught:
            success = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode',choices=['paired'],required=True)
    parser.add_argument('--arm',choices=['B'],required=True)
