...
from guarded_runner import GuardedRunner, Interrupts
from prepare import PACKET, SHA, TREE, digest, verify_harness
from supervise import bundle
from validate import verify_source, verify_build, save
...
def main():
    ...
    parser.add_argument('stage',choices=['prepare','build','run','floor'])
    ...
    assert json.loads((PACKET/'SOURCE-READY.json').read_text())['sourceReady'] is True, 'preparation only; sourceReady is false'
    verify_harness()
    ...
    with Interrupts() as interrupts:
        runner=GuardedRunner(PACKET,receipts,env,interrupts,free_floor=FREE,
            packet_ceiling=CAP,log_ceiling=LOG,overall_seconds=3600 if args.stage=='run' else (180 if args.stage=='floor' else 1500),reserve=30)
        ...
        if args.stage=='build':
            ...
            runner.run('compile-001',['cmake','--build',str(PACKET/'build'),'--target','RetentionForegroundProbe','RetentionFloorRaceProbe','--parallel','1'],cwd=PACKET,timeout=1200)
        ...
        if args.stage=='floor':
            runner.run('floor-race-001',[sys.executable,str(HERE/'run-floor-race.py'),
                '--probe',str(PACKET/'build/RetentionFloorRaceProbe'),
                '--build-proof',str(PACKET/'BUILD-PROOF.json'),
                '--packet',str(PACKET/'floor-race-run-001')],cwd=PACKET,timeout=120)
            verify_source(); verify_harness(); verify_build()
            return
        ...
        # Exactly one fresh instrumented B. Original timing triplets and
        # successful correctness controls remain in run35365051287 unchanged.
        schedule=[{'label':'phases-B-001','mode':'paired','arm':'B','instrumented':True}]
        ...
        for item in schedule:
            ...
            runner.run(label,[sys.executable,str(HERE/'supervise.py'),'--probe',str(probe),
                ...
                '--mode',item['mode'],'--arm',item['arm']],cwd=fixture,timeout=120)
        ...
        save(PACKET/'RESULT.json',{'status':'one-shot instrumented retention phase trace collected',
            ...
            'productPerformanceAcceptance':'not measured; diagnostic trace is separate from retained uninstrumented timing'})
