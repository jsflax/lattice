...
def authenticate_repository(runner, label, repository, expected, *, allowed_changes=(), initial=False, status_only_changes=()):
    def git(suffix, *argv):
    ...
    for entry in filter(None, status):
        if entry[3:] not in (*allowed_changes, *status_only_changes):
...
DIAGNOSTIC_SDK = 'af7775a269d87c37f2ad9a25a5aa60bbcac7b88f'
DIAGNOSTIC_CORE = '234afb4ad4f74174bd875035e6bb47b1a878ac56'
DIAGNOSTIC_OVERLAY_SHA256 = '89f3578a86bb75439b13c21609c3e67fda810b5c6583d21f7e89db17b46f42b3'
DIAGNOSTIC_IDS = (
    'LatticeTests.ObserverPushTests/teardownRaceStressUnderCommitLoad()',
    'LatticeTests.URLChangeSyncTests/sameURLDoesNotKick()',
    'LatticeTests.Vec0LockStormTests/test_NearestCountPerformance()',
)
DIAGNOSTIC_FILTER = '^(?:' + '|'.join(re.escape(x) for x in DIAGNOSTIC_IDS) + r')(?:/|$)'


def apply_diagnostic_overlay(packet, sdk, initial):
    manifest_path = packet / 'overlay.json'
    if manifest_path.is_symlink() or digest(manifest_path) != DIAGNOSTIC_OVERLAY_SHA256:
        raise ValueError('diagnostic overlay manifest differs from reviewed exact input')
    manifest = json.loads(manifest_path.read_text())
    if initial['commit'] != manifest['sdkCommit'] or initial['tree'] != manifest['sdkTree']:
        raise ValueError('diagnostic pristine SDK identity mismatch')
    expected = dict(initial['files'])
    # Validate every pre/postimage before any write. The fixed manifest names only three tests.
    for name, row in manifest['files'].items():
        source, destination = packet / 'overlay' / name, sdk / name
        if any(p.is_symlink() for p in [source, destination, *source.parents, *destination.parents]):
            raise ValueError('diagnostic source path contains a symlink: ' + name)
        if digest(source) != row['afterSHA256'] or digest(destination) != row['beforeSHA256']:
            raise ValueError('diagnostic test pre/postimage mismatch: ' + name)
        expected[name] = {'sha256': row['afterSHA256'], 'bytes': source.stat().st_size}
    for name in manifest['files']:
        shutil.copyfile(packet / 'overlay' / name, sdk / name)
    return expected, tuple(manifest['files'])


def check_diagnostic_discovery(log):
    selected = [line.strip() for line in log.read_text().splitlines()
                if re.match(DIAGNOSTIC_FILTER, line.strip())]
    if Counter(selected) != Counter(DIAGNOSTIC_IDS):
        raise ValueError('diagnostic discovery does not contain each exact selected ID once: ' + repr(selected))
    return selected


def check_diagnostic_results(log, xml):
    text = log.read_text()
    tree = ET.parse(xml)
    cases = [(node.get('classname'), node.get('name')) for node in tree.iter('testcase')]
    expected = [tuple(identifier.split('/', 1)) for identifier in DIAGNOSTIC_IDS]
    if Counter(cases) != Counter(expected):
        raise ValueError('diagnostic XML must contain exactly the three selected cases: ' + repr(cases))
    if any(node.tag in ('failure', 'error', 'skipped') for node in tree.iter()):
    ...
    for node in tree.iter():
        if node.tag in ('testsuite', 'testsuites'):
            for key in ('failures', 'errors', 'skipped', 'disabled'):
                if key in node.attrib and int(node.attrib[key]) != 0:
                    raise ValueError('diagnostic XML aggregate reports ' + key)
    names = Counter(identifier.split('/', 1)[1] for identifier in DIAGNOSTIC_IDS)
    starts = Counter(re.findall(r'^◇ Test (?!run )(.+) started\.$', text, re.M))
    passes = Counter(re.findall(r'^✔ Test (?!run with )(.+) passed after [0-9.]+ seconds?\.$', text, re.M))
    summaries = re.findall(r'^✔ Test run with (\d+) tests?(?: in \d+ suites?)? passed after [0-9.]+ seconds?\.$', text, re.M)
    if starts != names or passes != names or summaries != ['3']:
        raise ValueError('diagnostic log lacks exact three starts/passes/terminal summary')
    if re.search(r'no matching test|no tests? (?:found|executed)|[✘↷]|recorded an issue|with \d+ issues?', text, re.I):
        raise ValueError('diagnostic log reports no selection, skip or issue')
    return {'identifiers': list(DIAGNOSTIC_IDS), 'logSHA256': digest(log), 'xmlSHA256': digest(xml),
            'started': sum(starts.values()), 'passed': sum(passes.values()), 'skipped': 0,
            'terminalTests': 3, 'fullSuiteQualified': False}


def check_diagnostic_receipts(runner):
    # GuardedRunner.records deliberately contains summaries; inspect the full one-shot receipts.
    for summary in runner.records:
        record = json.loads((runner.receipts / (summary['label'] + '.json')).read_text())
        cleanup = record.get('cleanup', {})
        log = runner.receipts / (summary['label'] + '.log')
        if record.get('logSHA256') != digest(log) or record.get('logBytes') != log.stat().st_size:
            raise ValueError('diagnostic command log changed: ' + summary['label'])
        if (not record.get('success') or record.get('exitCode') != 0 or record.get('primaryError')
                or record.get('evidenceErrors') or record.get('receivedSignals') or record.get('stopReason')
                or not cleanup.get('leaderReaped') or not cleanup.get('groupGone') or cleanup.get('errors')
                or cleanup.get('signals')):
            raise ValueError('diagnostic full command receipt is not clean: ' + summary['label'])
...
def main():
    ...
    parser.add_argument('--diagnostic-selected-three-overlay', type=Path,
                        help='reviewed exact-af777 three-test packet; 1200-second diagnostic only')
    args = parser.parse_args()
    diagnostic = args.diagnostic_selected_three_overlay is not None
    if diagnostic and (args.core_sha != DIAGNOSTIC_CORE or args.test_timeout != 5400):
        raise ValueError('selected-three diagnostic requires exact Core234 and unchanged Linux full-suite setting5400')
    ...
    sdk_sha = DIAGNOSTIC_SDK if diagnostic else workflow_sha
    if not SHA.fullmatch(args.core_sha) or not SHA.fullmatch(sdk_sha) or not SHA.fullmatch(workflow_sha):
    ...
    result = {'scope': 'development source override only; not release qualification',
              ...
              'testMode': 'diagnostic-selected-three' if diagnostic else 'full',
              'diagnosticQualified': False, 'fullSuiteQualified': False,
    ...
    primary = None
    diagnostic_changes = ()
    diagnostic_acceptance = None
    expected_sdk_files = None
    ...
    with Interrupts() as interrupts:
        ...
        try:
            ...
            expected_sdk_files = sdk_inputs['files']
            if diagnostic:
                expected_sdk_files, diagnostic_changes = apply_diagnostic_overlay(
                    args.diagnostic_selected_three_overlay.resolve(strict=True), sdk, sdk_inputs)
                save_json(receipts / 'diagnostic-overlay.json', {
                    'manifestSHA256': DIAGNOSTIC_OVERLAY_SHA256,
                    'files': {name: expected_sdk_files[name] for name in diagnostic_changes}})
            ...
            if diagnostic:
                selected_common = ['swift', 'test', *common, '--force-resolved-versions', '--skip-build',
                                   '--disable-xctest', '--enable-swift-testing']
                discovery = runner.run('selected-three-discovery', [*selected_common, '--list-tests'],
                                       cwd=sdk, timeout=120, require_full_timeout=True)
                save_json(receipts / 'selected-three-discovered.json', check_diagnostic_discovery(discovery))
                selected = runner.run('selected-three-test', [*selected_common, '--filter', DIAGNOSTIC_FILTER,
                                      '--xunit-output', str(receipts / 'selected-three.xml')],
                                      cwd=sdk, timeout=1200, require_full_timeout=True)
                diagnostic_acceptance = check_diagnostic_results(selected, receipts / 'selected-three.xml')
                save_json(receipts / 'selected-three-acceptance.json', diagnostic_acceptance)
            else:
            ...
            final_sdk = authenticate_repository(runner, 'sdk-final', sdk, sdk_sha, allowed_changes=('Package.resolved',),
                                                status_only_changes=diagnostic_changes)
            final_core = authenticate_repository(runner, 'core-final', core, args.core_sha)
            initial_sdk_files = {k: v for k, v in expected_sdk_files.items() if k != 'Package.resolved'}
            ...
            with interrupts.hold():
                ...
                if diagnostic:
                    evidence('diagnostic full command receipts', lambda: check_diagnostic_receipts(runner))
                    def final_selected_evidence():
                        if diagnostic_acceptance is None or check_diagnostic_results(
                                receipts / 'selected-three-test.log', receipts / 'selected-three.xml') != diagnostic_acceptance:
                            raise ValueError('diagnostic selected result missing or changed')
                    evidence('diagnostic final selected evidence', final_selected_evidence)
                ...
                result['diagnosticQualified'] = result['success'] and diagnostic
                result['fullSuiteQualified'] = result['success'] and not diagnostic
                try:
                    save_json(receipts / 'RESULT.json', result)
                except BaseException as error:
                    result['success'] = False
                    result['diagnosticQualified'] = result['fullSuiteQualified'] = False
