...
private func nearestTimeoutPhase(_ phase: String, wave: Int = -1, slot: Int = -1) {
    let line = "BROAD_LINUX_PHASE test=Vec0LockStormTests.test_NearestCountPerformance pid=\(ProcessInfo.processInfo.processIdentifier) ns=\(DispatchTime.now().uptimeNanoseconds) phase=\(phase) wave=\(wave) slot=\(slot)\n"
...
class Vec0LockStormTests: BaseTest {
    ...
    func test_NearestCountPerformance() async throws {
        nearestTimeoutPhase("test_begin")
        defer { nearestTimeoutPhase("scope_exit") }
        Lattice.setLogLevel(.debug)
        let ownedNativeLogPath = try #require(ProcessInfo.processInfo.environment["LATTICE_TEST_LOG_PATH"])
        let performanceLogURL = URL(fileURLWithPath: ownedNativeLogPath)
            .deletingLastPathComponent().appendingPathComponent("nearest-count-performance.log")
        try? FileManager.default.removeItem(at: performanceLogURL)
        Lattice.setLogFile(performanceLogURL)
        nearestTimeoutPhase("open_begin")
        let lattice = try testLattice(Document.self)
        nearestTimeoutPhase("open_end")

        let dims = 128
        let docCount = 5000
        nearestTimeoutPhase("seed_begin")
        for i in 0..<docCount {
            let embedding = (0..<dims).map { _ in Float.random(in: -1...1) }
            try lattice.add(Document(title: "doc-\(i)", embedding: embedding))
        }

        nearestTimeoutPhase("seed_end")
        let k = 1000
        let query = FloatVector((0..<dims).map { _ in Float.random(in: -1...1) })
        let nearest = lattice.objects(Document.self)
            .nearest(to: query, on: \.embedding, limit: k)

        nearestTimeoutPhase("warm_query_before_begin")
        _ = nearest.count // warm up
        nearestTimeoutPhase("warm_query_before_end")
        ...
        nearestTimeoutPhase("before_queries_begin")
        ...
        nearestTimeoutPhase("before_queries_end")
        ...
        // --- Train IVF centroids ---
        nearestTimeoutPhase("ivf_train_begin")
        ...
        nearestTimeoutPhase("ivf_train_end")
        print("\n=== IVF training: \(String(format: "%.0f", trainMs)) ms ===\n")

        nearestTimeoutPhase("warm_query_after_begin")
        _ = nearest.count // warm up
        nearestTimeoutPhase("warm_query_after_end")

        // --- After IVF training ---
        nearestTimeoutPhase("after_queries_begin")
