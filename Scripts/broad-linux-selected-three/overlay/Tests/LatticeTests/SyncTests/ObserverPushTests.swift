...
private func teardownTimeoutPhase(_ phase: String, wave: Int = -1, slot: Int = -1) {
    let line = "BROAD_LINUX_PHASE test=ObserverPushTests.teardownRaceStressUnderCommitLoad pid=\(ProcessInfo.processInfo.processIdentifier) ns=\(DispatchTime.now().uptimeNanoseconds) phase=\(phase) wave=\(wave) slot=\(slot)\n"
...
final class ObserverPushTests: BaseTest {
    ...
    @Test(.timeLimit(.minutes(5))) func teardownRaceStressUnderCommitLoad() async throws {
        teardownTimeoutPhase("test_begin")
        defer { teardownTimeoutPhase("scope_exit") }
        teardownTimeoutPhase("harness_call_begin")
        try await withPushHarness { harness in
            teardownTimeoutPhase("body_begin")
            let manager = try #require(harness.watchHandle.pushManager)
            let file = harness.channelFile("g1")
            ...
            for waveIndex in 0..<waves {
                teardownTimeoutPhase("wave_begin", wave: waveIndex)
                ...
                teardownTimeoutPhase("connect_begin", wave: waveIndex, slot: 0)
                wave.append(try await harness.connect(
                    pathSuffix: "watch/group/g1", user: UUID(), on: doomedGroup))
                teardownTimeoutPhase("connect_end", wave: waveIndex, slot: 0)
                for subscriberSlot in 1..<subscribersPerWave {
                    teardownTimeoutPhase("connect_begin", wave: waveIndex, slot: subscriberSlot)
                    wave.append(try await harness.connect(pathSuffix: "watch/group/g1", user: UUID()))
                    teardownTimeoutPhase("connect_end", wave: waveIndex, slot: subscriberSlot)
                }
                ...
                for (subscriberSlot, collector) in wave.enumerated() {
                    teardownTimeoutPhase("initial_wait_begin", wave: waveIndex, slot: subscriberSlot)
                    #expect(await collector.wait(timeout: 30) { $0.receivedGlobalIds.count >= 1 })
                    teardownTimeoutPhase("initial_wait_end", wave: waveIndex, slot: subscriberSlot)
                }

                // Commit storm on its own thread; do NOT await it before closing.
                teardownTimeoutPhase("burst_launch", wave: waveIndex)
                let burst = Task.detached { [harness, waveIndex] () -> Int in
                    teardownTimeoutPhase("burst_begin", wave: waveIndex)
                    defer { teardownTimeoutPhase("burst_exit", wave: waveIndex) }
                ...
                teardownTimeoutPhase("group_shutdown_begin", wave: waveIndex)
                try? await doomedGroup.shutdownGracefully()
                teardownTimeoutPhase("group_shutdown_end", wave: waveIndex)
                for (remainingSlot, collector) in wave.dropFirst().enumerated() {
                    teardownTimeoutPhase("socket_close_begin", wave: waveIndex, slot: remainingSlot + 1)
                    try? await collector.socket?.close(code: .goingAway)
                    teardownTimeoutPhase("socket_close_end", wave: waveIndex, slot: remainingSlot + 1)
                }
                teardownTimeoutPhase("burst_join_begin", wave: waveIndex)
                minted += await burst.value
                teardownTimeoutPhase("burst_join_end", wave: waveIndex)
                // Let the teardown fully land before the next wave rebuilds the
                // group over the same file.
                try? await Task.sleep(nanoseconds: 50_000_000)
                teardownTimeoutPhase("wave_end", wave: waveIndex)
            }
            ...
            teardownTimeoutPhase("final_group_wait_begin")
            #expect(harness.diagnosticResult(await pollHasGroup(manager, file: file, equals: false)))
            teardownTimeoutPhase("final_group_wait_end")
            teardownTimeoutPhase("final_connect_begin")
            let after = try await harness.connect(pathSuffix: "watch/group/g1", user: UUID())
            teardownTimeoutPhase("final_connect_end")
            let co = try harness.coWriter("g1")
            try co.add(SimpleSyncObject(value: -1, floatValue: -1))
            ...
            teardownTimeoutPhase("final_delivery_wait_begin")
            #expect(await after.wait(timeout: 60) { $0.receivedGlobalIds.count >= expected.count })
            teardownTimeoutPhase("final_delivery_wait_end")
            #expect(after.receivedGlobalIds == expected)
            teardownTimeoutPhase("body_return")
        }
        teardownTimeoutPhase("harness_call_end")
    }
